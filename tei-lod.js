(function(window) {
    teilod = {
         ...
         */
        getLemmas : function(a_elem,a_callback) {
            $('#tei-lemmas-loading').html('Looking up lemmas....');
            var lang = teilod.getLanguageForElement(a_elem);
            if (lang != null && lang != '') {
                var url = teilod.morphUrl;
                url= url.replace('WORD',$(a_elem).val()).replace('LANG',lang).replace('ENGINE',teilod.morphEngines[lang]);
                $.ajax(
                        url, {
                        type: 'GET',
                        dataType: 'json',
                        success: function(data,textStatus,xR) {
                            ...
                            if (annotations && annotations[0] != null) 
                            {
                            ...
                            for (var i in parses) {
                                ...
                                for (var k in entries) {
                                    lemmas.push(entries[k].dict.hdwd.$);
                                }
                                 
                            }
                            a_callback($.unique(lemmas));
                            $('#tei-lemmas-loading').html('');
                        },
                        error: function(xR,textstatus,error) {
                            $('#tei-lemmas-loading').html('Error');
